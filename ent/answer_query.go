...
func (aq *AnswerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Answer, error) {
	...
	if query := aq.withQuestion; query != nil {
		if err := aq.loadQuestion(ctx, query, nodes,
			func(n *Answer) { n.Edges.Question = []*Question{} },
			func(n *Answer, e *Question) { n.Edges.Question = append(n.Edges.Question, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AnswerQuery) loadQuestion(ctx context.Context, query *QuestionQuery, nodes []*Answer, init func(*Answer), assign func(*Answer, *Question)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(answer.QuestionTable)
		s.Join(joinT).On(s.C(question.FieldID), joinT.C(answer.QuestionPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(answer.QuestionPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(answer.QuestionPrimaryKey[1]))
