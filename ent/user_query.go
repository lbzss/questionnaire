...
func (uq *UserQuery) QueryCreate() *QuestionnaireQuery {
	query := (&QuestionnaireClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithCreate(opts ...func(*QuestionnaireQuery)) *UserQuery {
	query := (&QuestionnaireClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withCreate; query != nil {
		if err := uq.loadCreate(ctx, query, nodes,
			func(n *User) { n.Edges.Create = []*Questionnaire{} },
			func(n *User, e *Questionnaire) { n.Edges.Create = append(n.Edges.Create, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadCreate(ctx context.Context, query *QuestionnaireQuery, nodes []*User, init func(*User), assign func(*User, *Questionnaire)) error {
	...
	query.withFKs = true
	query.Where(predicate.Questionnaire(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.CreateColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_create" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
