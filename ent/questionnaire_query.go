...
// QueryInclude chains the current query on the "include" edge.
func (qq *QuestionnaireQuery) QueryInclude() *QuestionQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, questionnaire.IncludeTable, questionnaire.IncludePrimaryKey...),
		)
...
func (qq *QuestionnaireQuery) QueryCreatedBy() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, questionnaire.CreatedByTable, questionnaire.CreatedByColumn),
		)
		fromU = sqlgraph.SetNeighbors(qq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (qq *QuestionnaireQuery) Clone() *QuestionnaireQuery {
	...
	return &QuestionnaireQuery{
		...
		withInclude:   qq.withInclude.Clone(),
		withCreatedBy: qq.withCreatedBy.Clone(),
...
// WithInclude tells the query-builder to eager-load the nodes that are connected to
// the "include" edge. The optional arguments are used to configure the query builder of the edge.
func (qq *QuestionnaireQuery) WithInclude(opts ...func(*QuestionQuery)) *QuestionnaireQuery {
	query := (&QuestionClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qq.withInclude = query
	return qq
}

// WithCreatedBy tells the query-builder to eager-load the nodes that are connected to
// the "created_by" edge. The optional arguments are used to configure the query builder of the edge.
func (qq *QuestionnaireQuery) WithCreatedBy(opts ...func(*UserQuery)) *QuestionnaireQuery {
	query := (&UserClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qq.withCreatedBy = query
	return qq
}
...
//		GroupBy(questionnaire.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Questionnaire.Query().
//		Select(questionnaire.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (qq *QuestionnaireQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Questionnaire, error) {
	var (
		...
		loadedTypes = [2]bool{
			qq.withInclude != nil,
			qq.withCreatedBy != nil,
		}
	)
	if qq.withCreatedBy != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := qq.withInclude; query != nil {
		if err := qq.loadInclude(ctx, query, nodes,
			func(n *Questionnaire) { n.Edges.Include = []*Question{} },
			func(n *Questionnaire, e *Question) { n.Edges.Include = append(n.Edges.Include, e) }); err != nil {
			return nil, err
		}
	}
	if query := qq.withCreatedBy; query != nil {
		if err := qq.loadCreatedBy(ctx, query, nodes, nil,
			func(n *Questionnaire, e *User) { n.Edges.CreatedBy = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (qq *QuestionnaireQuery) loadInclude(ctx context.Context, query *QuestionQuery, nodes []*Questionnaire, init func(*Questionnaire), assign func(*Questionnaire, *Question)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(questionnaire.IncludeTable)
		s.Join(joinT).On(s.C(question.FieldID), joinT.C(questionnaire.IncludePrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(questionnaire.IncludePrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(questionnaire.IncludePrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "include" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (qq *QuestionnaireQuery) loadCreatedBy(ctx context.Context, query *UserQuery, nodes []*Questionnaire, init func(*Questionnaire), assign func(*Questionnaire, *User)) error {
